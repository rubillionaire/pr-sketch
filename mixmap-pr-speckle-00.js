...
const drawTerrainMeshTile = map.createDraw({
  ...
  frag: glsl(`
    ...
    void main () {
      ...
      vec3 diffuse = diffusedLightAmount * color * clampedCosTheta;
      float randomThreshold = sqrt(random(vec2(random(vPosition.xy), random(vPosition.yz))));
      float cosThetaThreshold = clampedCosTheta - 0.005;
      if (randomThreshold < cosThetaThreshold) {
        gl_FragColor = vec4(ambient + diffuse + vec3(clampedCosTheta), 1);
      }
      else {
