...
function createMap ({ mix, name, viewbox=startViewbox }) {
...
function createDraws ({ map, radiatingCoastlineOpts={}, selectCity=0.0 }) {
  console.log('createDraws', map.viewbox)
  const globalContext = {
  ...
  const terrainImgTileLayer = ({ drawCmd, neGeojson }) => {
    console.log('terrainImgTileLayer : map._props()[0].viewbox', map._props()[0].viewbox)
    return {
      viewbox: (bbox, zoom, cb) => {
        cb(null, terrainImgTiles)
      },
      add: (key, bbox) => {
        ...
        const prop = Object.assign({}, map._props(), {
          key,
          id,
          zindex: 1,
          texture: map.regl.texture(),
  ...
  const drawCmds = {
    // ocean: map.createDraw(Object.assign({}, oceanShader, { id: 'ocean' })),
    // area: map.createDraw(Object.assign({}, geoRenderShaders.areas)),
    // coastlineShadow: map.createDraw(Object.assign({}, coastlineShadowShader)),
    terrainImgTile: map.createDraw(Object.assign({}, terrainImgTileShader)),
    ...
    // radiatingCoastline: map.createDraw(Object.assign({}, includeAllTags ? geoRenderShadersTick.lineFill : geoRenderShaders.lineFill)),
