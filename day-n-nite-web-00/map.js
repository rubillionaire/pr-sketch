...
async function createProps ({ maps, includeIsland=false }) {
  // terrain tiles : start
  ...
  for (const { tile, assetName } of terrainImgElements) {
    ...
    for (const { mixName, map } of maps) {
      ...
      terrainImgCmdProps.push({ mixName, prop })
    }
  }

  function terrainLayerProps ({ drawCmd, mixName }) {
    ...
    return {
      ...
      add: (key, bbox) => {
        ...
        const prop = terrainImgCmdProps.find(p => {
          return p.mixName === mixName &&
            p.prop.assetName === assetName
        }).prop
        prop.points = [
          bbox[0], bbox[1], // sw
          bbox[0], bbox[3], // se
...
function createDraws ({
    ...
    lineFill: Object.assign({}, geoRenderShaders.lineFill, {
      ...
      frag: glsl`
        ...
        void main () {
          ...
          float speckleHidden = step(x * 0.8 * tt, randomThreshold);

          float hasLight = step(0.0, dotSphereLight);
          vec3 colorHsluv = mix(colorBackground.xyz, colorForeground.xyz, hasLight);
          vec3 bgColor = mix(colorForeground.xyz, colorBackground.xyz, hasLight);
          vec3 color = colorHsluv.xyz;
          gl_FragColor = vec4(mix(color, bgColor, speckleHidden), 1.0);
        }
      `,
    })
  }
  ...
  if (!(disableDraws && disableDraws.ocean)) drawCmds.ocean = map.createDraw(oceanShader)
  if (!(disableDraws && disableDraws.coastlineShadow)) drawCmds.coastlineShadow = map.createDraw(coastlineShadowShader)
  if (!(disableDraws && disableDraws.terrainImgTile)) drawCmds.terrainImgTile = map.createDraw(terrainImgTileShader)
  if (!(disableDraws && disableDraws.radiatingCoastline)) drawCmds.radiatingCoastline = map.createDraw(includeAllTags ? geoRenderShadersTick.lineFill : geoRenderShaders.lineFill)
  if (!(disableDraws && disableDraws.city)) drawCmds.city = map.createDraw(cityShader)
...
function spreadProps ({ map, draw, props, mixName }) {
  const georenderProps = props.georender.find(g => g.mixName === mixName).georenderProps
  if (draw.terrainImgTile) {
    map.addLayer(props.terrainLayer({
      drawCmd: draw.terrainImgTile,
      mixName,
    }))
  }
  if (draw.radiatingCoastline) {
    setProps(draw.radiatingCoastline.props, georenderProps.lineP)  
  }
  if (draw.coastlineShadow) {
    setProps(draw.coastlineShadow.props, georenderProps.area)
  }
  if (draw.city) {
