// mixmap-05
// moves to hsl colors. should still make water distinct triangles
// to fit in with tie vibe of the ground
...
const drawTerrainMeshTile = map.createDraw({
  ...
  frag: glsl(`
    ...
    void main () {
      ...
      if (vPosition.w <= 0.0) {
        ...
        currentElevation = snoise(vPosition.xyz * tick/10000.0);
      };
      ...
      elevationColor.z = elevationColor.z - ((1.0 - lightAlignment) * 0.2);
  ...
  uniforms: {
    ...
    lightSource: ({ tick }) => {
      ...
      const rotated = vec3.rotateY([], lightSource, [0,0,0], tick/50)
      return rotated
    },
    tick: ({ tick }) => tick,
  },
