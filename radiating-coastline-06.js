...
const params = {
  ...
  lightPositionRealtime: searchParams.has('lightPositionRealtime') ? true : false
}
...
// terrain-img:start
let lightPositionTheta = ({ t }) => {
  return 2 * Math.PI * Math.sin(t)
}
if (params.lightPositionRealtime) {
  ...
  lightPositionTheta = () => {
    ...
    return ((2 * Math.PI * normalizedSecond) + Math.PI) % (2 * Math.PI)
  }
}
const terrainImgTileShader = {
  ...
  uniforms: {
    ...
    lightPosition: ({ tick }) => {
      const origin = [prCenterX, prCenterY, 0]
      const radius = [180, 1, 2000]
      const t = tick / 500
      // const theta = 2.0 * Math.PI * Math.abs(Math.sin(t))
      // const phi = Math.PI * Math.abs(Math.sin(t))
      const theta = lightPositionTheta({ t })
      const phi = Math.PI / 2
      const x = radius[0] * Math.sin(phi) * Math.cos(theta) + origin[0]
      const y = radius[1] * Math.sin(phi) * Math.sin(theta) + origin[1]
      const z = radius[2] * Math.cos(phi) + origin[2]
      return [x, y, z]
    },
  ...
  frag: `
    ...
    void main () {
      ...
      float lightDiffuseAmount = max(dot(positionNormal, lightDirection), 0.0);
      // if (lightDiffuseAmount < -0.3 && normalizedElevation > minElevation) {
      //   gl_FragColor = colorForeground;
      //   return;
      // }
