...
function addElevation ({ bgZindex, elevationZindex }) {
  ...
  const bg = {
    ...
    frag: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(colorBg, sample.a);
      }
    `
  }
  const elevation = {
    ...
    frag: `
      ...
      const vec2 zoomRange = vec2(8., 12.);
      ...
      void main () {
        ...
        float tileDim = mix(80.0, 700.0, z);
...
function addWaterBg ({ zindex }) {
  ...
  const draw = map.createDraw({
    ...
    frag: `
      ...
      varying vec2 vpos;
      void main () {
        float isFg = mod(vpos.y, 0.001);
        vec3 color = mix(
          colorFg,
          colorBg,
          step(isFg, 0.0004)
        );
        gl_FragColor = vec4(colorFg, 1.0);
      }
    `,
    blend: {
      enable: true,
...
addWaterBg({ zindex: 1 })
addElevation({ bgZindex: 2, elevationZindex: 3 })
