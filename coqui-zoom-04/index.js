...
const drawGlyphs = regl({
  ...
  frag: glsl`
    ...
    void main () {
      ...
      float reveal = 0.0;
      if (
          inRange(zoom1RangeOverscaled, showZoom) > 0.5 &&
          inRange(vec2(0.9, 1.1), labelZoom) > 0.5
        ) {
        // zoom 1 proper
        float zoom1Reveal = smoothstep(zoom1RangeOverscaled.x, zoom1RangeOverscaled.y, showZoom);
        reveal = 1.0;
      }
      else if (
          inRange(zoom1RangeOverscaled, showZoom) > 0.5 &&
          inRange(vec2(1.9, 2.1), labelZoom) > 0.5
        ) {
        // zoom 1 overlapping zoom 2
        float zoom1Label2Reveal = smoothstep(zoom2RangeOverscaled.x, zoom2RangeOverscaled.y, showZoom);
        reveal = step(1.0 - zoom1Label2Reveal, labelIndex/labelCount);
      }
      else if (
          inRange(zoom2RangeOverscaled, showZoom) > 0.5 &&
          inRange(vec2(1.9, 2.1), labelZoom) > 0.5
        ) {
        // zoom 2 proper
        float zoom2Label2Reveal = smoothstep(zoom2RangeOverscaled.x, zoom2RangeOverscaled.y, showZoom);
        reveal = step(1.0 - zoom2Label2Reveal, labelIndex/labelCount);
      }
      else if (
          inRange(zoom2RangeOverscaled, showZoom) > 0.5 &&
          inRange(vec2(2.9, 3.1), labelZoom) > 0.5
        ) {
        // zoom 2 overlapping zoom 3
        float zoom2Label3Reveal = smoothstep(zoom3Range.x, zoom3Range.y, showZoom);
        reveal = step(1.0 - zoom2Label3Reveal, labelIndex/labelCount);
      }
      else if (
          inRange(zoom3Range, showZoom) > 0.5 &&
          inRange(vec2(2.9, 3.1), labelZoom) > 0.5
        ) {
        // zoom 3 proper
        float zoom3Reveal = smoothstep(zoom3Range.x, zoom3Range.y, showZoom);
        reveal = step(1.0 - zoom3Reveal, labelIndex/labelCount);
      }
      else if (showZoom < 0.0) {
        reveal = 1.0;
      }
