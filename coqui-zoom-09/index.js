...
// - 09
// - hoist density out of the `dim` function so that each layer can define it individually
// - tweaks mark density at each zoom. less quite space
// - tweaks placement of mark 0, to be offset
...
function markLabelsZoom1 (mark) {
  ...
  const baseLabel = (opts={}) => {
    ...
    return {
      ...
      anchor: [0.4, -0.6 * (window.innerWidth / window.innerHeight)],
      speed: 0.005,
      fontSize,
...
function dim ({ zoom, density, mark, maxFontSize }) {
  let fontSize
  if (window.innerWidth < window.innerHeight) {
  ...
  const cols = Math.ceil(window.innerWidth / fontSize / density )
  const rows = Math.max(1, Math.ceil(window.innerHeight / fontSize / 2.0 / density ))
