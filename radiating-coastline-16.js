...
// - 15
// - remoove `glsl-hsluv` usage, we are not leaning into a dynamic color range
// - add coqui text to mountains to balanec out the night view
// - 16
// - turn off coqui text by default
// -- to see text: ?coqui=true || ?coqui=0,1,2,3
// - patches clipTile, had some bad logic, can be copied to previous versions
// if need be
// - adds terrainZoom flag to capture what level of terrain detail you want
// to load. 9 is default
// -- ?terrainZoom=8 || ?terrainZoom=10
// - adjust coastline shadow density, a little less dense mixes with the primary
// terrain a bit better
...
const campoJson = require('./util/pr-campo.json')
  ...
  lightPosition: !searchParams.has('lightPosition') || searchParams.get('lightPosition') === 'tick'
    ...
    : searchParams.get('lightPosition') === 'now'
      ...
      : isFloat(searchParams.get('lightPosition'))
        ...
        : 'tick',
  coqui: searchParams.has('coqui')
    ?  searchParams.get('coqui') === 'true'
      ? campoJson.map((d, i) => i)
      : searchParams.get('coqui').split(',').map(Number)
    : [],
...
const coquiShader = {
  ...
  uniforms: {
    ...
    charOffset: map.prop('charOffset'),
    tick: ({ tick }) => tick,
    radiusOffsetNormalized: map.prop('radiusOffset'),
    thetaOffset: map.prop('thetaOffset'),
    colorDark: colors.glslDark,
  ...
  vert: `
    ...
    uniform vec2 offset, anchor;
    uniform vec4 viewbox;
    uniform float zindex, aspect, charOffset, tick, radiusOffsetNormalized, thetaOffset;
    ...
    void main () {
      ...
      vec2 scale = vec2(0.00001);
      float maxRadius = 0.001;
      float radiusOffset = radiusOffsetNormalized * maxRadius;
      float speed = tick * 0.000006;
      float radius = mod(speed + radiusOffset, maxRadius);
      float normalizedRadius = radius/maxRadius;
      scale += normalizedRadius * 0.08;
      // char offset is range -1, 1
      // i want to map that onto [0, PI]
      float theta = mix(0.0, PI, 1.0 - smoothstep(-0.9, 0.9, charOffset));
      theta += thetaOffset;
      vec2 offsetRelativeAnchor = vec2(cos(theta) * radius, sin(theta) * radius - 0.05);
      mat2 rotation = rotate2d(theta - PI/2.0);
      vec2 p = ((position * flipY) * rotation) * scale + anchor + offsetRelativeAnchor + offset;
      gl_Position = vec4(
        (p.x - viewbox.x) / (viewbox.z - viewbox.x) * 2.0 - 1.0,
        ((p.y - viewbox.y) / (viewbox.w - viewbox.y) * 2.0 - 1.0) * aspect,
        1.0/(2.0+zindex), 1);
  ...
  frag: glsl`
    ...
    uniform vec2 anchor;
    uniform vec3 lightPosition;
    uniform vec4 colorLight, colorDark;
    ...
    void main () {
      vec3 positionSphere = lonLatToSphere(anchor);
      vec3 lightDirectionSphere = normalize(lightPosition - positionSphere);
      float dotSphereLight = dot(positionSphere, lightDirectionSphere);
      ...
      float randomThreshold = sqrt(random(vPos));
      float isHidden = step(randomThreshold, hiddenThreshold);
      vec4 color = mix(
        colorDark,
        colorLight,
        isHidden
...
async function loadCampo () {
  const draw = {
    coqui: map.createDraw(coquiShader),
  }
  ...
  const mark = 'coquí'
  const markMeshes = ({ radiusOffset=0.0, thetaOffset=0.0, anchor }={}) => {
  ...
  const props = campoJson
    .filter((d, i) => params.coqui.includes(i))
    .map(({ coordinates }) => {
      const radiusOffsetTick = Math.random()
      const radiusOffsetTock = (radiusOffsetTick + 0.5) % 1.0
      return markMeshes({
          anchor: coordinates,
          radiusOffset: radiusOffsetTick,
          thetaOffset: -Math.PI/20,
        })
        .concat(markMeshes({
          anchor: coordinates,
          radiusOffset: radiusOffsetTock,
          thetaOffset: +Math.PI/20,
        }))
    }).flat()

  draw.coqui.props = props
}
loadCampo()
...
const terrainImgTileManifest = terrainImgTileManifests[params.terrainZoom]
...
const terrainImgTileLayer = ({ drawCmd, neGeojson }) => {
  return {
    ...
    add: (key, bbox) => {
      ...
      const src = `terrain-rgb/${file}`
      // map.draw()
      resl({
        manifest: { tile: { type: 'image', src } },
        onDone: ({ tile }) => {
          const tileKey = file.split('.')[0].split('-').map(Number)
...
resl({
  ...
  onDone: async ({ neGeojson }) => {
    ...
    map.addLayer(terrainImgTileLayer({ drawCmd: draw.terrainImgTile, neGeojson }))
